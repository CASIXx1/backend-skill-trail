...
Scheduled job image を push する場合は、scheduled-log repository URL と Dockerfile を指定します。この1つのimageを `JOB_NAME` で heartbeat job と notification job に切り替えます。

```sh
ECR_REPOSITORY_URL="$SCHEDULED_LOG_ECR_REPOSITORY_URL" DOCKERFILE=Dockerfile.scheduled-log ./scripts/push-image.sh
```
...
OIDC trust policy は GitHub Environment `dev` の `sub` claim を許可します。API、migration、worker、scheduled-log の ECR repository URL は tfstate の output から取得します。
...
ecspresso --envfile .env --config ecspresso/scheduled-log/ecspresso.yml render config
ecspresso --envfile .env --config ecspresso/scheduled-log/ecspresso.yml render task-definition
ecspresso --envfile .env --config ecspresso/scheduled-notification/ecspresso.yml render config
ecspresso --envfile .env --config ecspresso/scheduled-notification/ecspresso.yml render task-definition
```
...
aws ecs register-task-definition --cli-input-json file:///tmp/scheduled-log-task-def.json
ecspresso --envfile .env --config ecspresso/scheduled-notification/ecspresso.yml render task-definition \
  | jq 'walk(if type == "object" and .versionConsistency == "" then del(.versionConsistency) else . end)' \
  > /tmp/scheduled-notification-task-def.json
aws ecs register-task-definition --cli-input-json file:///tmp/scheduled-notification-task-def.json
```

Scheduled jobs は長時間稼働する ECS service を作らず、task definition だけを登録します。EventBridge Scheduler はTerraform側で有効化します。GitHub Actions は Scheduler を直接更新せず、同じ image から `JOB_NAME=log_heartbeat` と `JOB_NAME=send_notification` の2つのtask definition revisionを登録します。SNS email subscription はTerraform apply後に確認メールが届くため、初回はメール本文の confirmation link を承認してください。
...
## Scheduled jobs smoke verification

Infra apply と backend deploy の完了後、heartbeat/notification task definition と Scheduler 起動結果を確認します。

```sh
aws ecs list-task-definitions --family-prefix "$SCHEDULED_LOG_TASK_DEFINITION_FAMILY" --status ACTIVE
aws ecs list-task-definitions --family-prefix "$SCHEDULED_NOTIFICATION_TASK_DEFINITION_FAMILY" --status ACTIVE
aws ecs list-tasks --cluster "$SCHEDULED_LOG_ECS_CLUSTER_NAME" --family "$SCHEDULED_LOG_TASK_DEFINITION_FAMILY" --desired-status STOPPED
aws ecs list-tasks --cluster "$SCHEDULED_NOTIFICATION_ECS_CLUSTER_NAME" --family "$SCHEDULED_NOTIFICATION_TASK_DEFINITION_FAMILY" --desired-status STOPPED
```

New Relic Logs では直近5分で以下の条件を確認します。

```text
event_type = 'scheduled_new_relic_heartbeat'
event_type = 'scheduled_notification_email'
```
