...
Scheduled-log image を push する場合は、scheduled-log repository URL と Dockerfile を指定します。
...
ecspresso --envfile .env --config ecspresso/scheduled-log/ecspresso.yml render task-definition > /tmp/scheduled-log-task-def.json
aws ecs register-task-definition --cli-input-json file:///tmp/scheduled-log-task-def.json
```

Scheduled-log は長時間稼働する ECS service を作らず、task definition だけを登録します。EventBridge Scheduler はTerraform側で有効化します。初回はinfra applyで無効化したまま作成し、backend deployでACTIVEなtask definition revisionを登録した後、infra側の `scheduled_log_scheduler_state` を `ENABLED` にして再度applyします。
...
## Scheduled-log smoke verification

Infra apply と backend deploy の完了後、scheduled-log task definition と Scheduler 起動結果を確認します。
